...
MPHouse(const char *name, const char *label)
  : images(),
    ...
    label((label) ? RNStrdup(label) : NULL),
...
ReadFile(const char *filename)
{
  ...
  if (!fp) { RNFail("Unable to open %s\n", filename); return 0; }
  fscanf(fp, "%s", file_type);
  fclose(fp);

  // Read file
  if (!strcmp(file_type, "ASCII")) return ReadAsciiFile(filename);
  else RNFail("Unable to read file %s, unrecognized type: %s\n", filename, file_type); 
...
ReadAsciiFile(const char *filename)
{
  ...
  FILE *fp = fopen(filename, "r");
  if (!fp) {
    RNFail("Unable to open house file %s\n", filename);
    return 0;
  }

  // Useful variables
  char cmd[512], version[512], name_buffer[512], label_buffer[512];
  ...
  if (strcmp(cmd, "ASCII")) {
    RNFail("Unable to read ascii file %s, wrong type: %s\n", filename, cmd);
    return 0;
  }
  ...
  // Fill in house info
  this->name = (strcmp(name_buffer, "-")) ? RNStrdup(name_buffer) : NULL;
  this->label = (strcmp(label_buffer, "-")) ? RNStrdup(label_buffer) : NULL;
  this->bbox = box;
  ...
  for (int i = 0; i < nlevels; i++) {
    ...
    if (strcmp(cmd, "L")) { RNFail("Error reading level %d\n", i); return 0; }
    MPLevel *level = new MPLevel();
    level->position = position;
    level->label = (strcmp(label_buffer, "-")) ? RNStrdup(label_buffer) : NULL;
    level->bbox = box;
    InsertLevel(level);
  }
  ...
  for (int i = 0; i < nregions; i++) {
    ...
    if (strcmp(cmd, "R")) { RNFail("Error reading region %d\n", i); return 0; }
    MPRegion *region = new MPRegion();
    region->position = position;
    region->label = (strcmp(label_buffer, "-")) ? RNStrdup(label_buffer) : NULL;
    region->bbox = box;
    region->height = (height > 0) ? height : box.ZMax() - position.Z();
    InsertRegion(region);
    if (level_index >= 0) {
  ...
  for (int i = 0; i < nportals; i++) {
    ...
    for (int j = 0; j < 4; j++) fscanf(fp, "%d", &dummy);
    if (strcmp(cmd, "P")) { RNFail("Error reading portal %d\n", i); return 0; }
    MPPortal *portal = new MPPortal();
    portal->span.Reset(p0, p1);
    portal->label = (strcmp(label_buffer, "-")) ? RNStrdup(label_buffer) : NULL;
    InsertPortal(portal);
    if (region0_index >= 0) {
  ...
  for (int i = 0; i < nsurfaces; i++) {
    ...
    fscanf(fp, "%lf%lf%lf%lf%lf%lf", &box[0][0], &box[0][1], &box[0][2], &box[1][0], &box[1][1], &box[1][2]);
    for (int j = 0; j < 5; j++) fscanf(fp, "%d", &dummy);
    if (strcmp(cmd, "S")) { RNFail("Error reading surface %d\n", i); return 0; }
    MPSurface *surface = new MPSurface();
    surface->position = position;
    surface->normal = normal;
    surface->label = (strcmp(label_buffer, "-")) ? RNStrdup(label_buffer) : NULL;
    surface->bbox = box;
  ...
  for (int i = 0; i < nvertices; i++) {
    ...
    if (strcmp(cmd, "V")) { RNFail("Error reading vertex %d\n", i); return 0; }
    MPVertex *vertex = new MPVertex();
    vertex->position = position;
    vertex->normal = normal;
    vertex->label = (strcmp(label_buffer, "-")) ? RNStrdup(label_buffer) : NULL;
    InsertVertex(vertex);
    if (surface_index >= 0) {
  ...
  for (int i = 0; i < npanoramas; i++) {
    ...
    fscanf(fp, "%lf%lf%lf", &position[0], &position[1], &position[2]);
    for (int j = 0; j < 5; j++) fscanf(fp, "%d", &dummy);
    if (strcmp(cmd, "P")) { RNFail("Error reading panorama %d\n", i); return 0; }
    MPPanorama *panorama = new MPPanorama();
    panorama->position = position;
    panorama->name = (strcmp(name_buffer, "-")) ? RNStrdup(name_buffer) : NULL;
  ...
  for (int i = 0; i < nimages; i++) {
    ...
    fscanf(fp, "%lf%lf%lf", &position[0], &position[1], &position[2]);
    for (int j = 0; j < 5; j++) fscanf(fp, "%d", &dummy);
    if (strcmp(cmd, "I")) { RNFail("Error reading image %d\n", i); return 0; }
    ...
    image->name = (strcmp(name_buffer, "-")) ? RNStrdup(name_buffer) : NULL;
  ...
  for (int i = 0; i < ncategories; i++) {
    ...
    for (int j = 0; j < 5; j++) fscanf(fp, "%d", &dummy);
    if (strcmp(cmd, "C")) { RNFail("Error reading category %d\n", i); return 0; }
    ...
    if (strcmp(label_name, "-")) category->label_name = RNStrdup(label_name);
    if (strcmp(mpcat40_name, "-")) category->mpcat40_name = RNStrdup(mpcat40_name);
  ...
  for (int i = 0; i < nobjects; i++) {
    ...
    if (strcmp(cmd, "O")) { RNFail("Error reading object %d\n", i); return 0; }
  ...
  for (int i = 0; i < nsegments; i++) {
    ...
    fscanf(fp, "%lf%lf%lf%lf%lf%lf", &box[0][0], &box[0][1], &box[0][2], &box[1][0], &box[1][1], &box[1][2]);
    for (int j = 0; j < 5; j++) fscanf(fp, "%d", &dummy);
    if (strcmp(cmd, "E")) { RNFail("Error reading segment %d\n", i); return 0; }
...
ReadCategoryFile(const char *filename)
{
  ...
  if ((label_id_k < 0) || (label_name_k < 0) || (mpcat40_id_k < 0)) {
    RNFail("Did not find index, raw_category, and mpcat40index in header of %s\n", filename);
    return 0;
  }
  ...
  while (fgets(value_buffer, 4096, fp)) {
    ...
    category->label_name = RNStrdup(values[label_name_k]);
    category->mpcat40_id = atoi(values[mpcat40_id_k]);
    category->mpcat40_name = RNStrdup(values[mpcat40_name_k]);
...
ReadSegmentFile(const char *filename)
{
  ...
  if (!json_root.isMember("segIndices")) {
    RNFail("Segment file %s has no segIndices\n", filename);
    return 0;
  }
...
ReadObjectFile(const char *filename)
{
  ...
  if (!json_root.isMember("segGroups")) {
    RNFail("Annotation file %s has no groups\n", filename);
    return 0;
  }
...
ReadConfigurationFile(const char *filename) 
{
  ...
  for (int i = 0; i < configuration.NImages(); i++) {
    ...
    strncpy(image_name_buffer, image->DepthFilename(), 1023);
    ...
    img->name = (name && strcmp(name, "-")) ? RNStrdup(name) : NULL;
    ...
    if (panorama) {
      ...
      panorama->name = (name && strcmp(name, "-")) ? RNStrdup(name) : NULL;
